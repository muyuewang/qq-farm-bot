...
const petDiaryActivityEnabled = computed(() => showPetDiaryActivity.value && [
  'pet_diary_adopt', 'pet_diary_feed', 'pet_diary_draw', 'pet_diary_story_claim', 'pet_diary_seed_claim',
  'pet_diary_solar_claim', 'pet_diary_treasure_open', 'pet_diary_compensation_claim', 'pet_diary_charm_equip', 'pet_diary_battle',
].some(key => automation.value.automation[key]))
...
function summaryTags(key: ModuleKey) {
  ...
  return [
    ...
    !starFestivalEnabled.value && (!SHOW_QIXI_ACTIVITY || !qixiActivityEnabled.value) && !rainPoemActivityEnabled.value && !petDiaryActivityEnabled.value && '未开启活动',
  ].filter(Boolean)
}
        ...
        <section class="max-h-[94vh] max-w-4xl w-full flex flex-col overflow-hidden border border-gray-200 rounded-lg bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800">
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">
            ...
            <div v-else class="space-y-4">
              ...
              <section class="space-y-3 border border-amber-100 rounded-lg p-4 dark:border-amber-900/40">
                <div>
                  <div class="text-sm text-gray-700 font-medium dark:text-gray-300">萌宠成长日记</div>
                  <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">领养、投喂、寻宝、手记、种子、节令、宝藏与好友夺宝</div>
                </div>
                <div class="grid gap-3 sm:grid-cols-2">
                  <BaseSwitch v-model="automation.automation.pet_diary_adopt" label="自动领养比熊" />
                  <BaseSwitch v-model="automation.automation.pet_diary_feed" label="自动投喂" />
                  <BaseSwitch v-model="automation.automation.pet_diary_draw" label="自动寻宝" />
                  <BaseSwitch v-model="automation.automation.pet_diary_story_claim" label="自动领取手记" />
                  <BaseSwitch v-model="automation.automation.pet_diary_seed_claim" label="自动领取种子礼包" />
                  <BaseSwitch v-model="automation.automation.pet_diary_solar_claim" label="自动领取节令小礼" />
                  <BaseSwitch v-model="automation.automation.pet_diary_treasure_open" label="自动领取宝藏" />
                  <BaseSwitch v-model="automation.automation.pet_diary_compensation_claim" label="自动领取夺宝补偿" />
                  <BaseSwitch v-model="automation.automation.pet_diary_charm_equip" label="自动选择锦囊" />
                  <BaseSwitch v-model="automation.automation.pet_diary_battle" label="自动好友夺宝（会掠夺好友）" />
                </div>
                <p class="text-xs text-amber-600 dark:text-amber-400">自动夺宝会消耗挑战书并掠夺好友护送中的宝藏，请确认后开启。</p>
              </section>
            </div>
          </div>
